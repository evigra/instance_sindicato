...
class Portalchildren(CustomerPortal):
    ...
    @http.route(
        ['/my/child/image/<int:partner_id>'],
        type='http',
        auth='user',
        website=True
    )
    def child_image(self, partner_id, **kw):

        user_partner = request.env.user.partner_id

        child = request.env['res.partner'].sudo().search([
            ('id', '=', partner_id),
            ('parent_contact_id', '=', user_partner.id),
        ], limit=1)

        if not child or not child.image_1920:
            return request.not_found()

        image = base64.b64decode(child.image_1920)
    ...
    @http.route(
        ['/my/children/<int:child_id>/credencial'],
        type='http',
        auth='user',
        website=True,
        methods=['GET', 'POST'],
    )
    def children_credencial(self, child_id, **post):
        ...
        values = {
            ...
            'child': child,
            'company': company,
            'page_name': 'children_credencial',
        }

        return request.render(
            'instance_sindicato.portal_children_credencial',
            values
        )
